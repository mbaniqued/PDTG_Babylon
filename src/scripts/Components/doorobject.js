...
import {Vector3,Color3,Angle,ActionManager,ExecuteCodeAction} from 'babylonjs';
export default class DoorObject{
            ...
            mesh.actionManager.registerAction(new ExecuteCodeAction(ActionManager.OnPickTrigger, (object)=> {
                        ...
                        switch(this.state){
                            case 0:
                                ...
                                this.root.setFocusOnObject(new Vector3(this.meshRoot.position.x,this.meshRoot.position.y,this.meshRoot.position.z+.8));
                                break;
                            case 1:
        ...
        openCloseDoor(eventcall){
            let val=325;
            if(this.meshRoot.rotation.y<= Angle.FromDegrees(325).radians())
                val=360;
           if(!this.anim){     
                this.anim=true;
                ...
                new TWEEN.Tween(this.meshRoot.rotation).to({y:Angle.FromDegrees(val).radians()},ANIM_TIME).easing(TWEEN.Easing.Sinusoidal.Out).onComplete(() => {
                    this.anim=false;
                    if(this.closedoor){
                        this.label.isVisible=false;
                        if(eventcall){
        ...
        setDoor(){
            this.meshRoot.rotation.y = Angle.FromDegrees(325).radians();
        }
        setLabel(){
