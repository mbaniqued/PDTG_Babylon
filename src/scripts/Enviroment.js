...
import {Scene,Viewport, Vector3,Color3,Angle,DirectionalLight,HemisphericLight,ArcRotateCamera } from "babylonjs";
...
export default class Enviroment{
     constructor(root){
        ...
        this.camVector     = new Vector3(0,3.2,0);
     }
     createScene(name) {
        this.scene = new Scene(this.game.engine);
        this.scene.name = name;
        this.scene.clearColor = Color3.FromHexString("#000000");
        
      //   this.env = this.scene.createDefaultEnvironment();
      //   this.env.setMainColor(new Color3.FromInts(255,253,227));
     ...
     createCamera(scene) {
        this.camera = new ArcRotateCamera("maincamera",0,0,10,this.camVector,scene);
        this.camera.fov   = start_fov;
        this.camera.layerMask=1;
        this.camera.inputs.clear();
        this.camera2  = new ArcRotateCamera("blureffectcamera",0,0,10,this.camVector,scene);
      ...
      setLight() {
        this.hemiLight              = new HemisphericLight("HemiLight",new Vector3(-100,100,-100),this.scene);
      //   this.hemiLight.groundColor  = new Color3.FromInts (255,255,255);
        this.hemiLight.diffuse      = new Color3.FromInts (255,253,227);
        this.hemiLight.specular     = new Color3.FromInts (0,0,0);
        
        this.directionalLight             = new DirectionalLight("DirectionalLight", new Vector3(100,-100,-40), this.scene);
        this.directionalLight.diffuse     = new Color3.FromInts (255,253,227);
        this.directionalLight.specular    = new Color3.FromInts (0,0,0);
      }
      setView(){
            this.hemiLight.intensity         = LIGHTOFF;
            this.directionalLight.intensity  = LIGHTOFF;
            this.camera.fov   = start_fov;
            this.camera.setTarget(new Vector3(-4,3,-.5));
            this.camera.position.set(0,0,0);
            this.camera.lowerRadiusLimit = 0;
            this.camera.upperRadiusLimit = 100;
            this.camera.radius          = 3;
            this.camera.alpha           = Angle.FromDegrees(230).radians();
            this.camera.beta            = Angle.FromDegrees(90).radians();
            this.camera.lowerBetaLimit  = Angle.FromDegrees(0).radians();
            this.camera.upperBetaLimit  = Angle.FromDegrees(180).radians();
      }
      updateCam(){
            // console.log("!!! updateCam!!! "+Angle.FromRadians(this.camera.alpha).degrees());
            ...
            if(this.scene.pointerX>0 && this.scene.pointerX<=this.camDirection.margin){
                  this.camera.alpha += Angle.FromDegrees(this.camDirection.deltaVal).radians();
                  if(this.camera.alpha>Angle.FromDegrees(359).radians())
                  ...
                  if(this.camera.alpha<0)
                        this.camera.alpha = Angle.FromDegrees(359).radians();
                        
            }
            else if(this.scene.pointerY>0 && this.scene.pointerY<=this.camDirection.margin){
                  if(this.camera.beta<1.7)
                        this.camera.beta += Angle.FromDegrees(this.camDirection.deltaVal).radians();
            }
        ...
        createminiMapCamera(){
            this.miniMapCam = new ArcRotateCamera("minimapcamera",0,0,10,this.minimapTarget,this.scene);
            this.miniMapCam.viewport = new Viewport(0.75,0.6,0.16,.2);
            // this.miniMapCam.viewport.height =  this.miniMapCam.viewport.width*1.12;
            this.miniMapCam.target = new Vector3(0,0,0);
            this.miniMapCam.position.set(0,0,0);
            this.miniMapCam.detachControl(this.game.canvas,false);  
            this.miniMapCam.alpha=Angle.FromDegrees(0).radians();
            this.miniMapCam.beta=Angle.FromDegrees(100).radians();
        ...
        setminiCamTarget(objecttype){
            ...
            switch(objecttype){
                  case 0:
                        ...
                        break;
                  case 1:
                        this.miniMapCam.alpha=Angle.FromDegrees(270).radians();
